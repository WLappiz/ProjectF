...
const eventsHandler = async (client, eventsPath) => {
            ...
            if (event.name) {
                ...
                console.log(
                    chalk.green.bold('SUCCESS: ') +
                        `Loaded event: ${chalk.cyan.bold(event.name)} from ${chalk.yellow(
                            getShortPath(file),
                        )}`,
                );
            } else {
                console.warn(
                    chalk.yellow.bold('WARNING: ') +
                        `File ${chalk.yellow(
                            getShortPath(file),
                        )} does not export a valid event name.`,
                );
            }
        } catch (error) {
            console.error(
                chalk.red.bold('ERROR: ') +
                    `Failed to load event from ${chalk.yellow(getShortPath(file))}:`,
                error,
            );
    ...
    const loadAllmodels = (modelsDir) => {
        const schemaFiles = getFilesRecursively(modelsDir);
        schemaFiles.forEach((file) => loadSchema(file));
    };

    loadAllEvents(eventsPath);
    loadAllmodels(path.join(__dirname, '../models'));

    const watcher = chokidar.watch([eventsPath, path.join(__dirname, '../models')], {
        persistent: true,
        ignoreInitial: true,
        awaitWriteFinish: true,
        ignored: [path.join(__dirname, '../../functions/**')],
    });
